...
class Aniversary(commands.Cog):
    ...
    @aniversay.command(name="embed", description="Cria um embed personalizavel de aniversario com menu interativo.")
    @app_commands.describe(
        title="Titulo do embed (obrigatorio)",
        description="Descricao do embed (obrigatorio)",
        color_hex="Cor em hexadecimal, exemplo: #FFAA00",
        footer="Texto do footer (opcional)",
        thumbnail_url="URL da thumbnail (opcional)",
        image_url="URL da imagem principal (opcional)",
        author_name="Nome do autor do embed (opcional)",
        author_icon_url="Icone do autor do embed (opcional)",
    ...
    async def aniversary_embed(
        ...
        if color_hex and color is None:
            await interaction.response.send_message(
                "Cor invalida. Use formato hexadecimal de 6 digitos, como #FFAA00.",
                ephemeral=True,
            )
            return
